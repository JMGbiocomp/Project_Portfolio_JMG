...
# A threshold for the bidirectional magnitude of the PC loading for the first three PCs determines the major contributors
...
# Arguments:
  ...
  # qc_data = logical value to determine if plots are generated for sample QC methods (reconstruction error, local outlier detection, and mahalanobis)
  # data_return = logical value to determines if the pca object is returned; default set to FALSE
  # load_threshold = numeric value between 0 and 1 as a cut off for magnitudinal direction of pca loading; default set to 0.5 (recommend evaluating loading plots prior to adjusting)
...
analysisPCA2 = function (data_object = NULL, feature_labels = NULL, center_data = TRUE, scale_data = FALSE, plot_data = TRUE, qc_data = TRUE, data_return = FALSE, load_threshold = 0.5, noise_filter = 0.8, verbose = FALSE) {
  # Errors, warnings, and messages
  if (is.null(data_object)) {stop("Must provide a matrix-like data structure with feature of interest in rows for PCA.")}
  if (is.null(feature_labels) | length(feature_labels) != dim(data_object)[2]) {stop("Must provide a vector of feature labels of length equal to the number of rows in the data_object.")}
  if (verbose) {
  ...
  if (plot_data) {
    ...
    if (qc_data) {
      ...
      # Evaluating anomalies against the PCA score plots
