...
# Argument:
  ...
  # dist_calculation = character vector of distance calucaltion methods for evaluation passed to hclusteringObject function to generate dendrograms; choices for calcaultions are "Euclidean", "Poisson", or "Correlation"

library(corrplot)
linkageCorrelation = function (count_data = NA, feature_labels = NA, dist_calculation = NA, linakge_methods = c("ward.D", "ward.D2", "single", "complete", "average", "mcquitty", "median", "centroid"), correlation_method = "cophenetic") {
  # errors and flags
  ...
  for (l in linkage_methods) {
    dend = hclustering_object(count_data = count_data, feature_labels = feature_labels, distance_type = dist_calculation, linkage_method = l, dend_plot = FALSE) # dendrogram
